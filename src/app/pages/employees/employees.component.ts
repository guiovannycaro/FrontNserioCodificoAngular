...
export class EmployeesComponent implements AfterViewInit,OnInit{
  ...
  displayedColumns: string[] = ['id', 'lastname','firstname','title','titleofcourtesy','birthdate','hiredate','address',
    'city','region','postalcode','country','phone','mgrid','Acciones'];
...
private obtenerCustomers() {
  this.api.getEmployeesList().subscribe({
    next: (dato) => {
...
Swal.fire({
  ...
  if (result.isConfirmed) {
    this.api.eliminarEmployees(id).subscribe({
      next: (data) => {
