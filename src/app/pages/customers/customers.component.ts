...
export class CustomersComponent implements AfterViewInit,OnInit{
    ...
    displayedColumns: string[] = ['id', 'companyname','contactname','contacttitle','address','city','region','postalcode',
      'country','phone','fax','Acciones'];
  ...
  private obtenerCustomers() {
    this.api.getCustomersList().subscribe({
      next: (dato) => {
...
openDropProForm(id: number) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      this.api.eliminarCustomers(id).subscribe({
        next: (data) => {
